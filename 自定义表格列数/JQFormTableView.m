...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==self.dataArr.count-1) {
        ...
        cell.jq_bottomSpace.constant = self.jq_lineWidth;//（最后一行底部多了一条线，所以要留出该线的间距，按钮的文本才显示到正中间）
        
    }else{
        cell.jq_bottomLine.hidden = YES;
        cell.jq_bottomSpace.constant = CGFLOAT_MIN; //最小值，接近0
    }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == self.dataArr.count-1) {
        CGFloat lastRowHeight = model.cellHeight;
        lastRowHeight += self.jq_lineWidth?self.jq_lineWidth:1;
...
//    NSLog(@"zzhh---%@",NSStringFromCGRect(self.frame));
//    NSLog(@"self.contentSize.height---%f",self.contentSize.height);
    ...
    if (self.contentSize.height>SCREEN_HEIGHT) {
        return self.frame.size.height;  //tableview原有高度
    }
